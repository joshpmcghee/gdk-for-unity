...
Using a terminal of your choice - for example, PowerShell on Windows - navigate to `gdk-for-unity-fps-starter-project/` and run `spatial cloud upload <assembly_name>`. The `<assembly_name>` is a string of your choice made up of alphanumeric characters, `_`, `.`, and `-`. A valid upload command would look like this:
```
...
Hit the Launch button on the left, and follow the instructions in the modal dialog (you can skip Step 1 - the SpatialOS Launcher was previously installed during setup). The SpatialOS Launcher will download the game client for this deployment and launch it.
...
We can use it, for instance, to highlight where all the Simulated Players and Player Entities are in the world (note: it's not cool to identify where your friends are hiding).
